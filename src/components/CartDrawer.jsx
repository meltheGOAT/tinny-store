...
import { X, ShoppingBag, Trash2, Plus, Minus, ArrowRight, ShieldCheck, Sparkles, MapPin, Tag } from 'lucide-react';
...
export default function CartDrawer() {
  ...
  const handleApplyPromo = (e) => {
    ...
    } else if (cleanCode === 'TINNY10' || cleanCode === 'WORLD' || cleanCode === 'ABUJA') {
  ...
  const discountAmountUSD = discountPercent > 0 ? Math.round(cartSubtotalUSD * (discountPercent / 100)) : 0;
      ...
      >
        ...
        <div className="free-shipping-container">
          <div className="shipping-status-text">
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', color: 'var(--accent-gold-hover)', marginBottom: '0.2rem' }}>
              <Sparkles size={13} />
              <span><strong>FREE Same-Day Delivery</strong> for Abuja residents & environs</span>
            </div>
            {remainingForFreeShipping === 0 ? (
              ...
              <span style={{ fontSize: '0.7rem', color: '#666' }}>
                Worldwide delivery: Add <strong>{formatPrice(remainingForFreeShipping)}</strong> more for free express
              </span>
            )}
          </div>
        ...
        {cart.length > 0 ? (
          <div className="cart-items-scroll">
            ...
            {/* Promo Code Input */}
            <form onSubmit={handleApplyPromo} style={{ display: 'flex', gap: '0.4rem', marginTop: '0.4rem' }}>
              <input 
                type="text" 
          ...
          <div style={{ textAlign: 'center', padding: '3.5rem 1.5rem', color: 'var(--text-muted)' }}>
            <ShoppingBag size={38} style={{ margin: '0 auto 0.75rem', opacity: 0.25 }} />
            <h3 style={{ fontSize: '1rem', marginBottom: '0.35rem', color: '#212121' }}>Your Bag is Empty</h3>
            <p style={{ fontSize: '0.8rem', marginBottom: '1.25rem', color: '#666' }}>
        ...
        {cart.length > 0 && (
          <div className="cart-footer-box">
            {appliedPromo && (
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.35rem', fontSize: '0.8rem', color: 'var(--accent-gold-hover)', fontWeight: 700 }}>
                <span>Code {appliedPromo.code} ({appliedPromo.percent}% Off)</span>
            ...
            <p style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginBottom: '0.85rem' }}>
              Free Same-Day Delivery applied for Abuja. Secure encrypted checkout.
            </p>

            <button 
              className="btn btn-primary checkout-action-btn"
              onClick={() => showToast('Redirecting to 256-Bit Encrypted Checkout...', 'success')}
            >
              <span>Checkout • {formatPrice(finalSubtotalUSD)}</span>
              <ArrowRight size={15} />
            </button>

            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.35rem', marginTop: '0.75rem', color: '#777', fontSize: '0.7rem' }}>
